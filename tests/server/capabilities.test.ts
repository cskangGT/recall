...
describe('GET /api/capabilities', () => {
  it('reports closed doors on a server with no readers', async () => {
    ...
    expect(res.body).toEqual({ appleNotes: false, notion: false, condense: false, google: false });
  });
  ...
  it('reports exactly the doors the deps can open', async () => {
    ...
    expect(res.body).toEqual({ appleNotes: true, notion: false, condense: false, google: false });
  });
});
