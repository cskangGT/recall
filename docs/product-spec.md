...
Not an LLM call. Every memory's `text` is embedded (1536 dimensions). Category centroids are recomputed as the mean of member embeddings after every assignment change. Entity embeddings are not computed — entity matching is lexical.
...
```ts
{
  ...
  centroid: vector(1536) | null; // mean of member memory embeddings
...
- A single committed JSON file: `seed/workspace.json`
...
- Embeddings are **not** included in the seed file. Search and Ask against seed data use a small precomputed similarity matrix in `seed/similarity.json` keyed by memory-id pairs, plus a `seed/answers.json` of scripted answers for the 5 demo questions (§12.5).
...
Source type distribution: 9 text, 8 link, 5 screenshot. Every category must contain at least one of at least two different source types, so the map visibly represents mixed media.

### 12.4 The demo-critical seed condition

**`AI Tooling` is seeded to sit exactly one memory below the SPLIT threshold.**

- 9 memories (threshold is ≥ 8 — satisfied)
- Mean pairwise cosine similarity **0.635** (threshold is < 0.62 — *not yet* satisfied)
- The 9 memories divide cleanly into two latent themes: 5 about agent frameworks and orchestration, 4 about evaluation and observability
- 2-means over them yields clusters of 5 and 4, inter-centroid distance 0.19 (threshold > 0.15 — satisfied)

Adding the demo item (§15) contributes 2 memories, both landing in the evaluation cluster. `AI Tooling` reaches 11 memories, mean pairwise similarity drops to **0.598** — crossing below 0.62 — and SPLIT fires deterministically. The resulting clusters are 5 (agent frameworks) and 6 (evaluation), both comfortably above the minimum of 3.
...
- **AC-27** *(Demo-critical)* Ingesting `seed/demo-item.json` into the seed workspace deterministically triggers exactly one SPLIT on `AI Tooling`, producing two child categories with 5 and 6 memories and leaving `AI Tooling` itself intact with zero directly-attached memories. Verified by an automated test that runs on every commit.
...
- Embedding model, provided it is 1536-dimensional and consistent across the corpus
- Test framework
